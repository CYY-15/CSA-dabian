...
func Register(ctx *gin.Context) {
	...
	//判断用户名是否存在
	var u1 model.User
	database.DB.Where("username = ?", name).First(&u1)
	if u1.ID != 0 {
		ctx.JSON(http.StatusUnprocessableEntity, gin.H{
			"code":    422,
			"message": "用户已存在",
	...
	//判断手机号是否存在
	var user model.User
	database.DB.Where("telephone = ?", telephone).First(&user)
	if user.ID != 0 {
		ctx.JSON(http.StatusUnprocessableEntity, gin.H{
			"code":    422,
			"message": "用户已存在",
...
// GetQuestionWithAnswers 根据用户名获取问题列表
func GetQuestionWithAnswers(c *gin.Context) {

	questionID := c.Param("id")

	var question model.Question
	if err := database.DB.Preload("Answers").First(&question, questionID).Error; err != nil {
...
func UpdateQuestion(c *gin.Context) {
	...
	// 检查问题是否属于当前用户（示例中假设问题与用户的关联是通过Author字段）
	if question.UserName != database.GetCurrentUserName(c) {
		c.JSON(403, gin.H{"error": "Not authorized to update this question"})
		return
	}
...
func UpdateAnswer(c *gin.Context) {
	...
	// 检查该回答是否属于当前用户（示例中假设问题与用户的关联是通过Author字段）
...
func DeleteMyQuestion(c *gin.Context) {
	...
	//if len(answers) > 0 {
	//	if err := db.Delete(&answers).Error; err != nil {
	//		c.JSON(500, gin.H{"error": "Failed to delete answers"})
	//		return
	//	}
	//}
...
func DeleteMyAnswer(c *gin.Context) {
	...
	// 检查当前用户是否是问题的作者
	if answer.UserName != database.GetCurrentUserName(c) {
		c.JSON(403, gin.H{"error": "You don't have permission to delete this answer"})
		return
	}
